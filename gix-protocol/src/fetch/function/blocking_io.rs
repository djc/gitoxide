...
use crate::fetch::{
    Arguments, Context, Error, Negotiate, NegotiateOutcome, Options, Outcome, ProgressId, Tags, negotiate,
};
use crate::transport::client::blocking_io::{ExtendedBufRead, HandleProgress, Transport};

use super::{acquire_shallow_lock, add_shallow_args};
...
/// with the help of `crate::BlockingSendFlushOnDrop` which can wrap `transport`.
...
{
    let _span = gix_trace::coarse!("gix_protocol::fetch_blocking()");
