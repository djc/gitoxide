...
use crate::{Handshake, credentials, handshake::refs};
...
pub fn handshake_blocking<AuthFn, T>(
    mut transport: T,
    service: Service,
...
where
    ...
    T: Transport,
{
    let _span = gix_features::trace::detail!("gix_protocol::handshake_blocking()", service = ?service, extra_parameters = ?extra_parameters);
    let (server_protocol_version, refs, capabilities) = {
        ...
        let parsed_refs = match refs {
            Some(mut refs) => {
                ...
                Some(refs::from_v1_refs_received_as_part_of_handshake_and_capabilities_blocking(
                    &mut refs,
                    capabilities.iter(),
                )?)
            }
