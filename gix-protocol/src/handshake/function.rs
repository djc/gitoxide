...
pub mod async_io {
    use super::*;
    use crate::handshake::refs::async_io::from_v1_refs_received_as_part_of_handshake_and_capabilities;
        ...
        let (server_protocol_version, refs, capabilities) = {
            ...
            let parsed_refs = match refs {
                Some(mut refs) => {
                    ...
                    Some(
                        from_v1_refs_received_as_part_of_handshake_and_capabilities(&mut refs, capabilities.iter())
                            .await?,
                    )
                }
                None => None,
...
pub mod blocking_io {
    use super::*;
    use crate::handshake::refs::blocking_io::from_v1_refs_received_as_part_of_handshake_and_capabilities;
        ...
        T: Transport,
    {
        ...
        let (server_protocol_version, refs, capabilities) = {
            ...
            let result = transport.handshake(service, &extra_parameters);
            let SetServiceResponse {
                actual_protocol,
                capabilities,
                refs,
                ...
                Err(client::Error::Io(ref err)) if err.kind() == std::io::ErrorKind::PermissionDenied => {
                    ...
                    progress.step();
                    progress.set_name("handshake (authenticated)".into());
                    match transport.handshake(service, &extra_parameters) {
                        Ok(v) => {
                            authenticate(next.store())?;
                            Ok(v)
                        }
            ...
            let parsed_refs = match refs {
                Some(mut refs) => {
                    ...
                    Some(from_v1_refs_received_as_part_of_handshake_and_capabilities(
                        &mut refs,
                        capabilities.iter(),
                    )?)
                }
