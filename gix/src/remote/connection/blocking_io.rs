//! Blocking variant of [`Connection`] and [`Prepare`].
    ...
    T: Transport,
{
    ...
    pub(crate) transport: BlockingSendFlushOnDrop<T>,
    ...
    T: Transport,
{
    ...
    pub fn ref_map(
        ...
        let refmap = self.ref_map_by_ref(progress, options)?;
        let handshake = self
            .handshake
    ...
    T: Transport,
{
    ...
    pub fn prepare_fetch(
        ...
        let ref_map = self.ref_map_by_ref(progress, options)?;
        Ok(Prepare {
            con: Some(self),
            ref_map,
            dry_run: DryRun::No,
    ...
    T: Transport,
{
    ...
    #[allow(clippy::result_large_err)]
    pub fn receive<P>(mut self, progress: P, should_interrupt: &AtomicBool) -> Result<Outcome, Error>
    where
...
// Re-export BString to keep error variants compatible across modules.
#[allow(dead_code)]
